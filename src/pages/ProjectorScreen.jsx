    ...
    <div className="projector-container">
      <AnimatePresence mode="wait">
        {!currentQuiz.isActive && (
          <motion.div
            ...
            style={{ width: '100vw', height: '100vh', position: 'fixed', top: 0, left: 0, zIndex: 1000, background: 'var(--bg-main)', display: 'flex', justifyContent: 'center', alignItems: 'center' }}
          >
      ...
      <div className="projector-content-wrapper">
        ...
        <aside className="projector-sidebar glass-panel">
            ...
            <table className="pl-table">
              ...
              <tbody>
                {sortedTeams.map((team, index) => {
                  ...
                  const awardedData = awardKey ? activeAwards[awardKey] : null;
          ...
          {/* Original scoreboard kept for easy rollback
          <div className="persistent-scoreboard">
            {sortedTeams.map((team) => {
              ...
              return (
                <div 
                  key={team.id} 
                  className={`proj-team-score ${isFirstPlace ? 'first-place' : ''}`} 
                  style={{ borderLeftColor: team.color }}
                >
                  <span className="proj-team-name" style={{ color: team.color }}>{team.name}</span>
                  <span className="proj-team-points">{team.score}</span>
                </div>
              );
            })}
          </div>
